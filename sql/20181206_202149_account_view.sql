  ...
  (
    ...
    FROM HS014357.STAFF STA
        ...
        FROM (
               ...
               FROM
                 HS014357.STAFF STA
               WHERE STA.STAFF_CODE IN (SELECT DISTINCT STAFF_CODE FROM HS014357.VCOURSE_TEACHING)
               UNION ALL
               SELECT
                 STA.STAFF_CODE,
                 16 ROLE_CODE
               FROM
                 HS014357.STAFF STA
               WHERE STA.STAFF_CODE IN (SELECT DISTINCT STAFF_CODE FROM HS014357.COU_DOCENT)
               UNION ALL
               SELECT
                 STA.STAFF_CODE,
                 0 ROLE_CODE
        ...
        GROUP BY STAFF_CODE
           ...
           CROSS JOIN TABLE(CURRENT_SEMESTER()) Y
    ...
    SELECT
      ...
      'STUDENT' AS                         SOURCE
    FROM
      HS014357.STU_BASIS STU
        INNER JOIN HS014357.STU_REGISTER REG ON REG.RGNO = STU.RGNO
        LEFT JOIN EXTRA_STU_DATA EXT_STU_DATA ON STU.IDNO = EXT_STU_DATA.ID_NUMBER
        LEFT JOIN ROLE_EXTEND RE ON RE.SOURCE = 'STUDENT' AND RE.KEY = STU.IDNO
    WHERE
        REG.REG_FLAG = '1'
    UNION ALL
    SELECT
