<%--@elvariable id="autobiographyList" type="java.util.List<model.Autobiography"--%>
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data == 0) {
                ...
                window.location.replace("autobiography?page=" + data);
            }
        },
        error: function () {
            alert("刪除失敗");
...
function validate() {
    ...
    validation &= if_length_too_long(test_length_array, autobiography_field_length_array);
    if (validation === 0)
        return false;
    else
        return true;
...
<div class="container">
            ...
            <div class="panel-body">
                <table class="table table-bordered" style="text-align: center">
                    <tr>
                        ...
                        <th class="text-center">自傳檔案</th>
                        <th class="text-center">副檔案</th>
                        <th class="text-center">操作</th>
                    </tr>
                    <c:if test="${autobiographyList.size() > 0}">
                        <c:forEach var="i" begin="0" end="${autobiographyList.size()-1}">
                            <tr>
                                <td>${autobiographyList.get(i).term_year}</td>
                                <td>${autobiographyList.get(i).term_semester}</td>
                                <td>${autobiographyList.get(i).topic}</td>
                                <td>${autobiographyList.get(i).description}</td>
                                <td>${autobiographyList.get(i).getCreatedDateString()}</td>
                                <td>
                                    <a href="${autobiographyList.get(i).main_link}">
                                            ${autobiographyList.get(i).main_uploadFile.fileName}
                                </td>
                                <td>
                                    <a href="${autobiographyList.get(i).sub_link}">
                                            ${autobiographyList.get(i).sub_uploadFile.fileName}
                                </td>
                                <td>
                                    <button onclick="deleted('${autobiographyList.get(i).id}')" class="btn btn-warning">
                                        刪除
                                    </button>
                                    <button onclick="modify('${autobiographyList.get(i).id}')" class="btn btn-warning">
                                        修改
                                    </button>
                                </td>
                            </tr>
    ...
    <div class="">
        <div class="panel panel-primary">
            <div class="panel panel-heading"><span class="glyphicon glyphicon-pencil" aria-hidden="true">　</span>新增自傳
            </div>
            <div class="panel-body">
                <div class="form-group">
                    <form name="myForm" action="autobiography" enctype='multipart/form-data' method="POST"
                        ...
                        <label for="file1" class="col-sm-4 required" style="font-size: large">自傳檔案上傳</label>
