...
@WebServlet("/autobiographydelete")
@MultipartConfig
public class AutobiographyDeleteServlet extends _BaseServlet {
...
//            response.sendRedirect("login");
        ...
        try {
            ...
            {
                if(autobiography.getMain_file_id()!=null && autobiography.getMain_file_id()!=0)
                    uploadFileDao.deleteById(autobiography.getMain_file_id());
                if(autobiography.getSub_file_id()!=null && autobiography.getSub_file_id()!=0)
                    uploadFileDao.deleteById(autobiography.getSub_file_id());
            }

            //刪除


            autobiographydao.deleteAutobiography(rgno, id);
            Integer after = autobiographydao.getPageNumber(rgno);
